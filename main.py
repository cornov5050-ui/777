...
async def setup_user_handlers(client, owner_id):
    ...
    async def slot_engine(e):
        if not e.out: return
        try: await e.delete() # Buyruqni o'chirish
        except: pass
        ...
        while running_tasks.get(task_key):
            try:
                ...
                # 64 qiymati 777 tushganini bildiradi
                if msg.media.value == 64:
                    running_tasks[task_key] = False
                    # Xabar linkini yasash
                    chat_peer = await client.get_input_entity(e.chat_id)
                    msg_id = msg.id
                    if hasattr(chat_peer, 'channel_id'):
                        link = f"https://t.me/c/{chat_peer.channel_id}/{msg_id}"
                    else:
                        link = f"https://t.me/c/{str(e.chat_id).replace('-100', '')}/{msg_id}"
                    
                    await bot.send_message(owner_id, f"🎊 **777 TUSHDI!**\n📍 **Link:** {link} 🔥")
                    break
                
                # Tezlikni oshirish uchun kichik pauza
